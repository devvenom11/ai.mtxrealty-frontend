  ...
  const loadData = async () => {
    ...
    apiCall("POST", "/api/load_data")
      .then(({ data, success, error }) => {
        ...
        toast.success(
          `${data.message}. Documents: ${data.docx_count}, PDFs: ${data.pdf_count}, Videos:${data.transcription_count}`
        );
      })
    ...
    >
      ...
      <label className="block">
        <input
          ...
          className="w-full px-4 py-2 text-gray-800 border border-gray-300 rounded !bg-white"
...
const AddYoutubeUrl = ({ loadData }) => {
  ...
  return (
    <form
      ...
      className="bg-white p-6 rounded-lg shadow-md mr-auto max-w-lg w-[450px]"
    >
...
const AddPDF = ({ loadData }) => {
  ...
  return (
    <form
      ...
      className="bg-white p-6 rounded-lg shadow-md ml-auto max-w-lg w-[450px]"
    >
